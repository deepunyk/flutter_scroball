...
public class Scrobbler {
  ...
  private final dev.dsi.flutter_scroball.LastfmClient client;
  private final ScrobbleNotificationManager notificationManager;
  private final ScroballDB scroballDB;
  private final ConnectivityManager connectivityManager;
  private final dev.dsi.flutter_scroball.TrackLover trackLover;
  private final EventBus eventBus = dev.dsi.flutter_scroball.ScroballApplication.getEventBus();
  private final List<PlaybackItem> pendingPlaybackItems;
  private final List<dev.dsi.flutter_scroball.Scrobble> pending;
  private boolean isScrobbling = false;
  private long lastScrobbleTime = 0;
  private long nextScrobbleDelay = 0;

  public Scrobbler(
      dev.dsi.flutter_scroball.LastfmClient client,
      ScrobbleNotificationManager notificationManager,
      ScroballDB scroballDB,
      ConnectivityManager connectivityManager,
      dev.dsi.flutter_scroball.TrackLover trackLover) {
    this.client = client;
  ...
  public void updateNowPlaying(dev.dsi.flutter_scroball.Track track) {
    ...
    client.updateNowPlaying(
        track,
        message -> {
          dev.dsi.flutter_scroball.LastfmClient.Result result = (dev.dsi.flutter_scroball.LastfmClient.Result) message.obj;
          int errorCode = result.errorCode();
          if (dev.dsi.flutter_scroball.LastfmClient.isAuthenticationError(errorCode)) {
            notificationManager.notifyAuthError();
            dev.dsi.flutter_scroball.ScroballApplication.getEventBus().post(dev.dsi.flutter_scroball.AuthErrorEvent.create(errorCode));
          }
          return true;
        });
  }
  ...
  public void submit(PlaybackItem playbackItem) {
    ...
    dev.dsi.flutter_scroball.Track track = playbackItem.getTrack();
    ...
    for (int i = playbackItem.getPlaysScrobbled(); i < playCount; i++) {
      ...
      dev.dsi.flutter_scroball.Scrobble scrobble = dev.dsi.flutter_scroball.Scrobble.builder().track(track).timestamp(itemTimestamp).build();
        ...
        message -> {
          if (message.obj == null) {
            ...
            if (errorCode == 6) {
              ...
              if (dev.dsi.flutter_scroball.LastfmClient.isTransientError(errorCode)) {
          ...
          dev.dsi.flutter_scroball.Track updatedTrack = (dev.dsi.flutter_scroball.Track) message.obj;
  ...
  public void scrobblePending() {
    ...
    isScrobbling = true;
    final List<dev.dsi.flutter_scroball.Scrobble> tracksToScrobble = new ArrayList<>(pending);
    ...
    client.scrobbleTracks(
        tracksToScrobble,
        message -> {
          List<dev.dsi.flutter_scroball.LastfmClient.Result> results = (List<dev.dsi.flutter_scroball.LastfmClient.Result>) message.obj;
          boolean shouldBackoff = false;

          for (int i = 0; i < results.size(); i++) {
            dev.dsi.flutter_scroball.LastfmClient.Result result = results.get(i);
            dev.dsi.flutter_scroball.Scrobble scrobble = tracksToScrobble.get(i);
  ...
  @Subscribe
  public void onTrackLoveEvent(dev.dsi.flutter_scroball.TrackLoveEvent e) {
